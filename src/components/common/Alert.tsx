...
export const Alert: React.FC<AlertProps> = ({ 
  ...
  const getAlertStyles = () => {
    const baseStyles = 'p-4 rounded-lg border-l-4 relative';
...
// Alert simples sem ícone
export const SimpleAlert: React.FC<{ message: string; type: AlertType; className?: string }> = ({ 
  message, 
  type, 
  className = '' 
}) => {
  const getSimpleAlertStyles = () => {
    const baseStyles = 'p-3 rounded text-sm';
  ...
  return (
    <div className={`${getSimpleAlertStyles()} ${className}`}>
      {message}
    </div>
  );
};
